...
namespace DevBot.Game
{
    public class GameBehavior
    {
        ...
        public GameBehavior(GameClient game)
        {
            ...
            m_ai = new GameAI(Game, m_duel);
            m_executor = Executor.GetExecutor(m_ai, m_duel);
            m_ai.Executor = m_executor;
            Deck = Deck.Load(m_executor.Deck);
        }
        ...
        private void RegisterPackets()
        {
            ...
            m_packets.Add(StocMessage.Chat, OnChat);
        ...
        private void OnTypeChange(GameServerPacket packet)
        {
            ...
            int pos = type & 0xF;
            if (!Game.IsCheckmate && pos != 0 && pos != 1)
            {
                Connection.Close();
                return;
            }
        ...
        private void OnPlayerChange(GameServerPacket packet)
        {
            ...
            if (!Game.IsCheckmate && m_room.IsHost && m_room.IsReady[0] && m_room.IsReady[1])
